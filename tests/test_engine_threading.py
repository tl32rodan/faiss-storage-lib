...
class TestFaissEngineThreading(unittest.TestCase):
    def test_each_thread_gets_dedicated_sqlite_connection(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def worker() -> None:
                engine._doc_store._get_conn().execute("SELECT 1")
                conn_id = id(engine._doc_store._get_conn())
                with lock:
            ...
            main_conn_id = id(engine._doc_store._get_conn())
    ...
    def test_add_delete_and_persist_operations_use_write_lock(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            original_next_int_id = engine._doc_store.next_int_id

            def assert_locked_next_int_id() -> int:
                self.assertTrue(engine._write_lock.locked())
                return original_next_int_id()

            engine._doc_store.next_int_id = assert_locked_next_int_id  # type: ignore[method-assign]
            engine.add([VectorDocument(uid="doc-1", vector=[0.1, 0.2], payload={"k": "v"})])

            original_fetch_int_ids = engine._doc_store.fetch_int_ids

            def assert_locked_fetch_int_ids(uids: list[str]) -> dict[str, int]:
                self.assertTrue(engine._write_lock.locked())
                return original_fetch_int_ids(uids)

            engine._doc_store.fetch_int_ids = assert_locked_fetch_int_ids  # type: ignore[method-assign]
            engine.delete(["doc-1"])

            original_write_index = engine._vector_store._faiss.write_index

            def assert_locked_write_index(index, path):
                self.assertTrue(engine._write_lock.locked())
                return original_write_index(index, path)

            engine._vector_store._faiss.write_index = assert_locked_write_index
            engine.persist()
            engine._vector_store._faiss.write_index = original_write_index
            engine.close()
